...
static u8 s_controller_type[SerialInterface::MAX_SI_CHANNELS] = { ControllerTypes::CONTROLLER_NONE };
static bool s_controller_calibrated[SerialInterface::MAX_SI_CHANNELS] = { false };
...
static void Read()
{
  ...
  while (s_adapter_thread_running.IsSet())
  {
    int err = libusb_interrupt_transfer(s_handle, s_endpoint_in, s_controller_payload_swap,
                                        sizeof(s_controller_payload_swap), &payload_size, 32);
    if (err)
...
static void Write()
{
  ...
  while (true)
  {
    ...
    int err =
        libusb_interrupt_transfer(s_handle, s_endpoint_out, payload, sizeof(payload), &size, 32);
    if (err)
